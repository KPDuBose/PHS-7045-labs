...
2. Design 1
  1) call postDraws
    1) Figure out which arm has the highest from beta distribution
    2) Get probability that one arm is the best
    3) Standardize
    4) Probability that arm is greater than control
    5) get max probability that an arm is greater than control
    6) return needed output
3. Design 2
  1) Define arms and looks
  2) set initial sample to have same size each arm
  3) account for residual in number of interim breaks
  4) Update allocation probabilities and nt for each look iteration and call post draws
